...
Console output only version - no email sending
"""
...
def run_all_strategies(strategies, strategy_manager, data_manager, downloader, live_prices=None, market_status=None):
    ...
    # Run each unique strategy only once
    for strategy_name in unique_strategies:
        # Check if strategy exists
        if (strategy_name not in strategy_manager.default_strategies and 
            strategy_name not in strategy_manager.test_strategies):
            print(f"\n전략 {strategy_name}을 찾을 수 없습니다. marketwatch.json을 확인하세요.")
            strategy_results[strategy_name] = None
            continue
        
        print(f"\n{'='*70}")
        print(f"분석 중: {strategy_name}")
        print('='*70)
...
def print_summary_report(results):
    """Print a summary report of all strategy results - grouped by strategy with recipient count"""
    print("\n\n" + "="*80)
    print(" OOSIT MARKET WATCH - 전체 전략 요약 보고서")
    print("="*80)
    ...
    # Get market status from first result
    market_status = None
    if sorted_strategies and sorted_strategies[0][1]['result'].get('market_status'):
        market_status = sorted_strategies[0][1]['result']['market_status']
    
    if market_status:
        print(f"시장 상태: {market_status}")
    
    print(f"\n총 {len(sorted_strategies)}개 전략 분석 완료")
    print("(수신자가 많은 전략부터 표시)")
    
    # Print each strategy's results grouped
    for strategy_name, data in sorted_strategies:
        ...
        print(f"수신자 ({recipient_count}명): {', '.join(sorted(recipients))}")
        print('='*70)
        
        if historical:
            print(f"\n1. 과거 데이터 기준:")
            print(f"   - 모드: {historical['mode']}")
            print(f"   - 포지션: {format_position(historical['position'])}")
        
        if current:
            print(f"\n2. 실시간 데이터 반영:")
            print(f"   - 모드: {current['mode']}")
            print(f"   - 포지션: {format_position(current['position'])}")
            
            # Check if mode changed
            if historical and current['mode'] != historical['mode']:
                print(f"\n   *** 모드 변경: {historical['mode']} → {current['mode']} ***")
        
        # Show live prices if available
        if res.get('latest_prices'):
            print(f"\n3. 실시간 가격:")
            price_strings = []
            for ticker, price in sorted(res['latest_prices'].items()):
                price_strings.append(f"{ticker}: ${price:.2f}")
            print("   " + ", ".join(price_strings))
    
    print("\n" + "="*80)
    print(f"모든 전략 분석 완료: {len(sorted_strategies)}개 전략, 총 {sum(len(d['recipients']) for _, d in sorted_strategies)}명 수신자")
    print("="*80)


def main():
    """메인 실행 함수"""
    print("--- 실시간 전략 분석 마켓워치 (콘솔 출력 버전) ---")
    print("(전체 가능한 데이터를 yfinance에서 다운로드하여 최근 3년을 분석합니다)")
    print("이메일 발송 없이 콘솔에만 결과를 출력합니다.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Print summary report instead of sending emails
